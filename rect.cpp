...
bool Rect::isOverlapping(const Rect &r) const {
    ...
    if(r.getLeftX()>getRightX()||r.getRightX()<getLeftX()){
        return false;
    }
    // 2. when one is above the other
    if(r.getTopY()>getBottomY()||r.getBottomY()<getTopY()){
        return false;
    }
...
void Rect::draw(double angle) {


    glColor3f(fill.red, fill.green, fill.blue);
    glBegin(GL_QUADS);

    double d=sqrt(size.width*size.width+size.height*size.height)/2;
    glVertex2d(center.x-cos(acos(size.width/2/d)-angle)*d,center.y-sin(asin(size.height/2/d)-angle)*d);
    glVertex2d(center.x-cos(acos(size.width/2/d)+angle)*d,center.y+sin(asin(size.height/2/d)+angle)*d);
    glVertex2d(center.x+cos(acos(size.width/2/d)-angle)*d,center.y+sin(asin(size.height/2/d)-angle)*d);
    glVertex2d(center.x+cos(acos(size.width/2/d)+angle)*d,center.y-sin(asin(size.height/2/d)+angle)*d);
